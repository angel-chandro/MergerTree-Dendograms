...
def cosgrowRhoCrit(z = 1, H0 = 100, OmegaM = 0.3, OmegaR = 0, w0 = -1, wprime = 0, Dist = "Ang", Mass = "Msun"): 
    ...
    OmegaL = 1 - OmegaM - OmegaR; #Munit = 1e10; Lunit = 1e6; Vunit = 1e3 # MODIFY
    G = 6.67384e-11; #msol_to_kg = 1.98892e+30; pc_to_m = 3.08568e+16; g = G * msol_to_kg/(pc_to_m); g = g * Munit/(Lunit * Vunit**2) # MODIFY
    km2m = 1000
    Mpc2m = 3.08567758e+22
    Msol2kg = 1.9891e+30
    Msun10=1e10
    Hub2 = cosgrowH(z = z, H0 = H0, OmegaM = OmegaM,OmegaR = OmegaR, w0 = w0, wprime = wprime)**2
    RhoCrit = (3 * Hub2)/(8 * np.pi * G) * (km2m**2)/Mpc2m**2; #RhoCrit = (3 * Hub2)/(8 * np.pi * g); # MODIFY * (km2m**2)/Mpc2m**2; 
    if (Mass == "Msun"): 
        RhoCrit = RhoCrit/(Msol2kg)
    if (Dist == "Ang"): 
        RhoCrit = RhoCrit * Mpc2m**3; # MODIFY RhoCrit = RhoCrit
    if (Dist == "Co"): 
...
def coshaloMvirToRvir(Mvir = 1e+12, z = 0, H0 = 100, OmegaM = 0.3, OmegaL=0.7, OmegaR = 0, Rho = "crit", Dist = "Co", DeltaVir = 200, Munit = 1e10, Lunit = 1e+06, Vunit = 1000, Dim = 3): 
    ...
    if (Rho == "crit"):
        RhoVal = (cosgrowRhoCrit(z = z, H0 = H0, OmegaM = OmegaM, OmegaR = OmegaR, Dist = Dist)/1e+09) * ((Lunit/1000)**3)/Munit; #RhoVal = cosgrowRhoCrit(z = z, H0 = H0, OmegaM = OmegaM, OmegaR = OmegaR, Dist = Dist); #MODIFY
